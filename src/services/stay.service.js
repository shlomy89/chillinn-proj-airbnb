    ...
    return storageService.query(STORAGE_KEY).then((stays) => {
        ...
        if (filterBy) {
            ...
            if (chosePropertyTypes.length) {
                stays = stays.filter(stay => chosePropertyTypes
                    .some(chosePropertyType => chosePropertyType === stay.propertyType))
            }

            const checkedPlaceTypes = Object.keys(placeTypes).filter(p => placeTypes[p])
            if (checkedPlaceTypes.length) {
                stays = stays.filter(stay => checkedPlaceTypes
                    .some(checkedPlaceType => checkedPlaceType === stay.placeType))
            }
